  ...
  return (
    <main className="min-h-screen bg-[#070d16] pb-16 text-slate-100">
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 py-7 md:px-6">
        ...
        {result && <section className="space-y-5"><div className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border border-emerald-400/25 bg-emerald-400/[0.08] p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-emerald-200">New master rows</p><p className="mt-1 text-3xl font-black text-white">{result.summary.new}</p></div><div className="rounded-xl border border-violet-400/25 bg-violet-400/[0.08] p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-violet-200">Merged 2nd squad</p><p className="mt-1 text-3xl font-black text-white">{result.summary.mergedSecondSquad}</p></div><div className="rounded-xl border border-rose-400/25 bg-rose-400/[0.08] p-4"><p className="text-xs font-bold uppercase tracking-[0.14em] text-rose-200">Errors</p><p className="mt-1 text-3xl font-black text-white">{result.summary.error}</p></div></div><div className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950/65"><div className="flex flex-wrap items-center gap-4 border-b border-white/10 p-5"><div className="min-w-0 flex-1"><h2 className="font-bold text-white">Coordinator preview</h2><p className="mt-1 text-sm text-slate-400">Errors remain out of MASTER_PASTE and appear in the ERRORS worksheet.</p></div><button onClick={downloadWorkbook} className="inline-flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-amber-300 to-orange-400 px-4 py-2.5 text-sm font-black text-slate-950 shadow-lg shadow-amber-500/15 transition hover:brightness-110"><Download className="h-4 w-4" />Download paste file</button></div><div className="max-h-[520px] overflow-auto"><table className="w-full min-w-[790px] text-left text-sm"><thead className="sticky top-0 bg-[#0d1729] text-[11px] uppercase tracking-[0.12em] text-slate-400"><tr><th className="px-4 py-3">Status</th><th className="px-4 py-3">Source</th><th className="px-4 py-3">Bowler</th><th className="px-4 py-3">ID</th><th className="px-4 py-3">Center / Team</th><th className="px-4 py-3">Squad / Note</th></tr></thead><tbody>{result.previewRows.map((row, index) => <tr key={`${row.status}-${row.sourceRows.join("-")}-${index}`} className="border-t border-white/[0.06]"><td className="px-4 py-3"><span className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-bold ${statusClass(row.status)}`}>{row.status}</span></td><td className="px-4 py-3 font-mono text-xs text-slate-400">{row.sourceRows.join(", ")}</td><td className="px-4 py-3 font-semibold text-white">{row.firstName} {row.lastName}</td><td className="px-4 py-3 font-mono text-xs text-cyan-200">{row.bowlerId || "—"}</td><td className="px-4 py-3 text-slate-300">{row.center || "—"}<span className="block text-xs text-slate-500">Team {row.team || "—"}</span></td><td className="px-4 py-3 text-slate-300">{row.squadTime || "—"}{row.reason && <span className="block max-w-sm text-xs leading-5 text-rose-200">{row.reason}</span>}</td></tr>)}</tbody></table></div></div></section>}
      </div>
    </main>
  );
}
