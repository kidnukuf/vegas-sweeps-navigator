...
import { CoordinatorHandoffPanel } from "@/components/CoordinatorHandoffPanel";
import { AdvertisingProspectsPanel } from "@/components/AdvertisingProspectsPanel";

type Readiness = { level: "ready" | "attention" | "blocked"; issues: string[] };
type EventDirector = { staffId: number; name: string; username: string; companyId?: number | null };
type EventRow = {
...
type OperationsData = {
  ...
  events: Array<{ id: number; eventName: string; eventYear: number; companyId: number | null; createdByStaffId: number | null; status: string }>;
  directors: Array<{ id: number; name: string; username: string; companyId: number | null; companyName: string | null; eventIds: number[]; eventCount: number }>;
...
function OwnerOperationsPanel({ data, onChanged, onOpenEvent }: { data: OperationsData; onChanged: () => void; onOpenEvent: (eventId: number) => void }) {
  ...
  const [eventDraft, setEventDraft] = useState({ eventName: "", eventYear: String(new Date().getFullYear()), companyId: "", assignedDirectorId: "", groupSlug: "bob", startDate: "", endDate: "", bowlingDate: "", squadTime: "", sheetSpreadsheetId: data.sharedSheet?.spreadsheetId ?? "", sheetTabName: "", sheetTabNickname: "" });
  const [directorDraft, setDirectorDraft] = useState({ name: "", username: "", password: "", companyId: "" });
  ...
  const createDirector = trpc.ownerDashboard.createDirector.useMutation({ onSuccess: () => { toast.success("Event Director credentials created"); setDirectorDraft({ name: "", username: "", password: "", companyId: "" }); onChanged(); }, onError: (error) => toast.error(error.message) });
  const assignExistingDirector = trpc.ownerDashboard.assignExistingEventDirector.useMutation({ onSuccess: ({ directorName }) => { toast.success(`${directorName} is now assigned to this event`); setExistingDirectorId(""); onChanged(); }, onError: (error) => toast.error(error.message) });
  ...
  const eligibleEventDirectors = data.directors.filter((director) => !eventDraft.companyId || director.companyId === Number(eventDraft.companyId));
  const existingEvent = data.events.find((event) => event.id === Number(existingEventId));
  const eligibleExistingDirectors = data.directors.filter((director) => !existingEvent?.companyId || director.companyId === existingEvent.companyId);
  const selectedDirectorEvents = data.events.filter((event) => event.createdByStaffId === selectedDirector?.id);
  ...
  return <section id="owner-operations" className="mb-8 overflow-hidden rounded-2xl border border-sky-300/15 bg-slate-950/65 shadow-2xl shadow-black/20">
      ...
      <div><p className="text-xs font-bold uppercase tracking-[0.18em] text-sky-300">Owner operations</p><h2 className="mt-1 text-xl font-semibold text-white">Create and assign</h2><p className="mt-1 max-w-2xl text-sm leading-6 text-slate-400">Create a planning event, assign its Event Director, issue Director credentials, and open any event from this portal. A selected Director becomes the event’s only Event Director workspace owner.</p></div>
      <div className="flex flex-wrap gap-2"><Button type="button" onClick={() => setShowEventCreate((shown) => !shown)} className="bg-amber-300 text-slate-950 hover:bg-amber-200"><CalendarPlus className="mr-2 h-4 w-4" />Create event</Button><Button asChild variant="outline" className="border-sky-300/30 bg-sky-300/10 text-sky-100 hover:bg-sky-300/20 hover:text-white"><a href="#owner-create-event-director"><UserPlus className="mr-2 h-4 w-4" />Add Event Director</a></Button></div>
    </div>

    <form id="owner-create-event-director" onSubmit={(event) => { event.preventDefault(); createDirector.mutate({ ...directorDraft, companyId: directorDraft.companyId ? Number(directorDraft.companyId) : undefined, eventIds: [] }); }} className="border-b border-sky-300/25 bg-sky-300/[0.05] p-5">
      <div className="mb-4"><h3 className="font-semibold text-white">Add Event Director</h3><p className="mt-1 text-sm text-slate-400">Create credentials here. New directors begin with no event access; each event they create is automatically theirs alone to manage. The password is hashed before storage and can be reset later.</p></div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Field label="Director name"><Input required className={inputClass} value={directorDraft.name} onChange={(event) => setDirectorDraft({ ...directorDraft, name: event.target.value })} /></Field><Field label="Username"><Input required className={inputClass} value={directorDraft.username} onChange={(event) => setDirectorDraft({ ...directorDraft, username: event.target.value })} placeholder="letters, numbers, . _ -" /></Field><Field label="Temporary password"><Input required type="password" minLength={8} className={inputClass} value={directorDraft.password} onChange={(event) => setDirectorDraft({ ...directorDraft, password: event.target.value })} /></Field><Field label="Company (optional)"><select className={`${inputClass} h-9 rounded-md px-3`} value={directorDraft.companyId} onChange={(event) => setDirectorDraft({ ...directorDraft, companyId: event.target.value })}><option value="">No company yet</option>{data.companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></Field></div>
    ...
    <form id="owner-assign-existing-event-director" onSubmit={(event) => { event.preventDefault(); assignExistingDirector.mutate({ eventId: Number(existingEventId), staffId: Number(existingDirectorId) }); }} className="border-b border-emerald-300/20 bg-emerald-300/[0.035] p-5">
      <div className="mb-4"><h3 className="font-semibold text-white">Assign existing Event Director to an existing event</h3><p className="mt-1 text-sm text-slate-400">This transfers the event’s Event Director workspace to the selected director. The previous director no longer has Event Director access to that event; Owner access remains unchanged.</p></div>
      <div className="grid gap-4 md:grid-cols-2"><Field label="Existing event"><select required className={`${inputClass} h-9 rounded-md px-3`} value={existingEventId} onChange={(event) => { const eventId = event.target.value; const selectedEvent = data.events.find((row) => String(row.id) === eventId); const currentDirectorEligible = data.directors.some((director) => String(director.id) === existingDirectorId && (!selectedEvent?.companyId || director.companyId === selectedEvent.companyId)); setExistingEventId(eventId); setExistingDirectorId(currentDirectorEligible ? existingDirectorId : ""); }}><option value="">Choose event</option>{data.events.map((event) => <option key={event.id} value={event.id}>{event.eventName} · {event.eventYear} · {event.status}</option>)}</select></Field><Field label="Existing Event Director"><select required disabled={!existingEventId || !eligibleExistingDirectors.length} className={`${inputClass} h-9 rounded-md px-3`} value={existingDirectorId} onChange={(event) => setExistingDirectorId(event.target.value)}><option value="">Choose Event Director</option>{eligibleExistingDirectors.map((director) => <option key={director.id} value={director.id}>{director.name} · {director.username}{director.companyName ? ` · ${director.companyName}` : ""}</option>)}</select><span className="normal-case text-xs font-normal tracking-normal text-slate-500">{existingEvent?.companyId ? "Only directors in this event’s company are eligible." : "This event has no company, so any existing Event Director is eligible."}</span></Field></div>
      <div className="mt-5 flex justify-end"><Button type="submit" disabled={!existingEventId || !existingDirectorId || assignExistingDirector.isPending} className="bg-emerald-300 text-slate-950 hover:bg-emerald-200">{assignExistingDirector.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <UserPlus className="mr-2 h-4 w-4" />}Assign Event Director</Button></div>
    </form>
    ...
    {showEventCreate && <form onSubmit={(event) => { event.preventDefault(); createEvent.mutate({ ...eventDraft, eventYear: Number(eventDraft.eventYear), companyId: Number(eventDraft.companyId), assignedDirectorId: eventDraft.assignedDirectorId ? Number(eventDraft.assignedDirectorId) : undefined }); }} className="border-b border-white/10 bg-white/[0.025] p-5">
      <div className="mb-4"><h3 className="font-semibold text-white">Create and assign a planning event</h3><p className="mt-1 text-sm text-slate-400">Select a Director from the event company to give that person the creator-scoped Event Director workspace immediately. Leave it unassigned only when the Owner will configure it first.</p></div>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
        ...
        <Field label="Company"><select required className={`${inputClass} h-9 rounded-md px-3`} value={eventDraft.companyId} onChange={(event) => { const companyId = event.target.value; const currentDirectorIsEligible = data.directors.some((director) => String(director.id) === eventDraft.assignedDirectorId && (!companyId || director.companyId === Number(companyId))); setEventDraft({ ...eventDraft, companyId, assignedDirectorId: currentDirectorIsEligible ? eventDraft.assignedDirectorId : "" }); }}><option value="">Choose company</option>{data.companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select></Field>
        <Field label="Event Director"><select className={`${inputClass} h-9 rounded-md px-3`} value={eventDraft.assignedDirectorId} onChange={(event) => setEventDraft({ ...eventDraft, assignedDirectorId: event.target.value })} disabled={!eventDraft.companyId}><option value="">Leave unassigned — Owner only</option>{eligibleEventDirectors.map((director) => <option key={director.id} value={director.id}>{director.name} · {director.username}</option>)}</select><span className="normal-case text-xs font-normal tracking-normal text-slate-500">Only Directors in the selected company can be assigned.</span></Field>
    ...
    <div className="p-5"><div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-sky-300">Event Director directory</p><h3 className="mt-1 text-lg font-semibold text-white">Creator-owned events and credential maintenance</h3><p className="mt-1 text-sm text-slate-400">Every Event Director is shown below, including directors who have not created an event yet.</p></div><select aria-label="Select Event Director to maintain" className={`${inputClass} h-9 rounded-md px-3 md:w-80`} value={selectedDirectorId} onChange={(event) => { setSelectedDirectorId(event.target.value); setResetPassword(""); }}><option value="">Select an Event Director</option>{data.directors.map((director) => <option key={director.id} value={director.id}>{director.name} · {director.username}</option>)}</select></div>
      <div className="mt-5 overflow-x-auto rounded-xl border border-white/10">
        <table className="min-w-full text-left text-sm">
          <thead className="bg-white/[0.045] text-xs uppercase tracking-[0.12em] text-slate-400"><tr><th className="px-4 py-3 font-semibold">Event Director</th><th className="px-4 py-3 font-semibold">Username</th><th className="px-4 py-3 font-semibold">Company</th><th className="px-4 py-3 text-right font-semibold">Events owned</th></tr></thead>
          <tbody className="divide-y divide-white/10">{data.directors.map((director) => <tr key={director.id} className="bg-slate-950/25 transition hover:bg-sky-300/[0.045]"><td className="whitespace-nowrap px-4 py-3 font-medium text-white">{director.name}</td><td className="whitespace-nowrap px-4 py-3 text-slate-300">{director.username}</td><td className="whitespace-nowrap px-4 py-3 text-slate-400">{director.companyName ?? "No company"}</td><td className="px-4 py-3 text-right"><span className="inline-flex min-w-8 justify-center rounded-full border border-sky-300/25 bg-sky-300/10 px-2.5 py-1 text-xs font-bold text-sky-100">{director.eventCount}</span></td></tr>)}{!data.directors.length && <tr><td colSpan={4} className="px-4 py-6 text-center text-slate-400">No Event Director accounts have been created yet.</td></tr>}</tbody>
        </table>
      </div>
      {selectedDirector && <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between"><div><p className="font-semibold text-white">{selectedDirector.name}</p><p className="mt-1 text-sm text-slate-400">{selectedDirector.username} · {selectedDirector.companyName ?? "No company"}</p></div><div className="flex w-full max-w-md gap-2"><Input type="password" minLength={8} className={inputClass} placeholder="New password (8+ characters)" value={resetPassword} onChange={(event) => setResetPassword(event.target.value)} /><Button disabled={resetPassword.length < 8 || resetDirectorPassword.isPending} onClick={() => resetDirectorPassword.mutate({ staffId: selectedDirector.id, password: resetPassword })} variant="outline" className="shrink-0 border-amber-300/30 text-amber-100 hover:bg-amber-300/10 hover:text-amber-50"><KeyRound className="mr-2 h-4 w-4" />Reset</Button></div></div>
        <div className="mt-4"><p className="text-sm font-semibold text-white">Events created by this director</p><p className="mt-1 text-xs text-slate-500">Only these events are visible to this Event Director. The Owner Portal remains able to open every event.</p><div className="mt-2 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{selectedDirectorEvents.map((event) => <div key={event.id} className="rounded-lg border border-white/10 bg-slate-950/45 p-3 text-sm text-slate-300"><span>{event.eventName}<span className="block text-xs text-slate-500">{event.eventYear} · {event.status}</span></span></div>)}{!selectedDirectorEvents.length && <p className="text-sm text-amber-200">This director has not created an event yet.</p>}</div></div>
      </div>}
    </div>
  </section>;
}
