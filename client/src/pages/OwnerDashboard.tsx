...
export default function OwnerDashboard() {
  ...
  return <div className="min-h-screen bg-[#080b14] text-slate-100 selection:bg-amber-300 selection:text-slate-950">
    ...
    <main className="relative mx-auto max-w-[1600px] px-4 py-6 md:px-8 md:py-10">
      <header className="mb-8 flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><button onClick={() => navigate("/")} className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-slate-400 transition hover:text-white"><ArrowLeft className="h-4 w-4" />Back to app</button><div className="flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-xl border border-amber-300/25 bg-amber-300/10"><ShieldCheck className="h-6 w-6 text-amber-300" /></span><div><p className="text-xs font-bold uppercase tracking-[0.24em] text-amber-300">Private workspace</p><h1 className="text-3xl font-semibold tracking-tight text-white md:text-4xl">Platform Owner Dashboard</h1></div></div><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">Validate readiness and edit every event across the platform. Access is tied to your Manus owner account: {user?.name ?? "Owner"}.</p></div><Button variant="outline" onClick={() => overview.refetch()} disabled={overview.isFetching} className="border-white/10 bg-white/[0.03] text-slate-200 hover:bg-white/[0.08] hover:text-white"><RefreshCw className={`mr-2 h-4 w-4 ${overview.isFetching ? "animate-spin" : ""}`} />Refresh validation</Button></header>
      ...
      <section className="overflow-hidden rounded-2xl border border-white/10 bg-slate-950/60 shadow-2xl shadow-black/20"><div className="flex flex-col gap-4 border-b border-white/10 p-4 md:flex-row md:items-center md:justify-between"><div><h2 className="text-lg font-semibold text-white">Cross-platform event readiness</h2><p className="mt-1 text-sm text-slate-500">Select an event to edit its settings and browse every bowler record.</p></div><div className="flex flex-col gap-2 sm:flex-row"><div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><Input className="w-full border-white/10 bg-white/[0.04] pl-9 text-white placeholder:text-slate-500 sm:w-64" placeholder="Search events" value={search} onChange={(e) => setSearch(e.target.value)} /></div><select className="h-9 rounded-md border border-white/10 bg-white/[0.04] px-3 text-sm text-slate-200" value={brand} onChange={(e) => setBrand(e.target.value)}><option value="all">All brands</option>{Object.entries(BRAND_LABELS).map(([slug, label]) => <option key={slug} value={slug}>{label}</option>)}</select></div></div>
