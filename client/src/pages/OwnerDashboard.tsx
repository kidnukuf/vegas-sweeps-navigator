...
import { ArrowLeft, CheckCircle2, ChevronRight, CircleAlert, ClipboardPenLine, ExternalLink, Loader2, Pencil, RefreshCw, Search, ShieldCheck, Trash2, TriangleAlert, Users } from "lucide-react";
...
type EventRow = {
  ...
  missingBanquetPasses: number; missingPoolPasses: number; sheetLastSyncedAt: number | null; readiness: Readiness;
};
...
export default function OwnerDashboard() {
  ...
  const filteredRows = useMemo(() => rows.filter((row) => {
    ...
    return matchesBrand && (!query || `${row.eventName} ${row.companyName ?? ""} ${row.groupSlug ?? ""}`.toLowerCase().includes(query));
  }), [brand, rows, search]);
  ...
  const refreshSelected = () => { detail.refetch(); overview.refetch(); };
  const doDelete = () => {
