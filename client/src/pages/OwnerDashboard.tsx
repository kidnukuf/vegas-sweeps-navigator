...
function OwnerOperationsPanel({ data, onChanged, onOpenEvent }: { data: OperationsData; onChanged: () => void; onOpenEvent: (eventId: number) => void }) {
  ...
  const createDirector = trpc.ownerDashboard.createDirector.useMutation({ onSuccess: () => { toast.success("Event Director credentials created"); setShowDirectorCreate(false); setDirectorDraft({ name: "", username: "", password: "", companyId: "" }); onChanged(); }, onError: (error) => toast.error(error.message) });
  ...
  useEffect(() => {
    if (!showDirectorCreate) return;
    const timeout = window.setTimeout(() => document.getElementById("owner-create-event-director")?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
    return () => window.clearTimeout(timeout);
  }, [showDirectorCreate]);

  const toggleDirectorCreate = () => {
    setShowDirectorCreate((shown) => !shown);
  ...
  return <section id="owner-operations" className="mb-8 overflow-hidden rounded-2xl border border-sky-300/15 bg-slate-950/65 shadow-2xl shadow-black/20">
      ...
      <div className="flex flex-wrap gap-2"><Button type="button" onClick={() => { setShowEventCreate((shown) => !shown); setShowDirectorCreate(false); }} className="bg-amber-300 text-slate-950 hover:bg-amber-200"><CalendarPlus className="mr-2 h-4 w-4" />Create event</Button><Button type="button" onClick={toggleDirectorCreate} aria-expanded={showDirectorCreate} aria-controls="owner-create-event-director" variant="outline" className="border-sky-300/30 bg-sky-300/10 text-sky-100 hover:bg-sky-300/20 hover:text-white"><UserPlus className="mr-2 h-4 w-4" />{showDirectorCreate ? "Close Event Director form" : "Add Event Director"}</Button></div>
    </div>
    ...
    {showDirectorCreate && <form id="owner-create-event-director" onSubmit={(event) => { event.preventDefault(); createDirector.mutate({ ...directorDraft, companyId: directorDraft.companyId ? Number(directorDraft.companyId) : undefined, eventIds: [] }); }} className="border-b border-sky-300/25 bg-sky-300/[0.05] p-5">
      <div className="mb-4"><h3 className="font-semibold text-white">Create Event Director credentials</h3><p className="mt-1 text-sm text-slate-400">New directors begin with no event access. Each event they create is automatically theirs alone to manage. The password is hashed before storage and can be reset later.</p></div>
