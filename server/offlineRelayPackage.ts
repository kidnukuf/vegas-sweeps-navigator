...
export function buildOfflineRelayGuide(): string {
  return [
    ...
    "4. Put the Pi scanner page full-screen. Scanner A captures rapid USB QR-scanner keystrokes globally, so no mouse click, cursor placement, or refocus is required between scans. Do not use the laptop monitor to scan QR codes.",
    "",
    "## During the event",
    "",
    "- Scanner A is the recommended one-scanner setup: plug in the USB QR scanner, open the exported page on the Pi, and scan continuously. The page clears each scan and remains ready.",
    "- Scanner B is optional and uses its dedicated input when operating a second scanner.",
