...
export const coordinatorRouter = router({
  ...
  admissionPasses: router({
    listMine: publicProcedure.input(z.object({ eventId: z.number().int().positive().optional() }).optional()).query(async ({ input, ctx }) => {
      const coordinator = await requireCoordinatorSession(ctx);
      const rows = await rawQuery<{
        ...
        under21: number | null; guestNames: string | null; guestCount: number; hasPoolPass: number; hasBanquetPass: number;
      }>(
        `SELECT e.id AS eventId, e.eventName, e.eventYear, e.startDate, e.endDate,
                ...
                b.scantronId AS bowlerIdLabel, b.under21, bc.centerName, t.teamName,
         ...
         WHERE EXISTS (
           SELECT 1 FROM coordinator_scopes s
           WHERE s.coordinatorAccountId = ? AND s.eventId = b.eventId
             AND (s.centerId IS NULL OR s.centerId = b.centerId)
             AND (? IS NULL OR s.eventId = ?)
         )
         ORDER BY bc.centerName, t.teamName, b.legalLastName, b.legalFirstName`,
        [coordinator.id, input?.eventId ?? null, input?.eventId ?? null],
      );
      return rows.map((row) => ({
        ...row,
